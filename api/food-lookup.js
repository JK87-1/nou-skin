    ...
    const jsonTemplate = `---JSON_START---
{
  ...
  "tags": ["키워드1", "키워드2", "키워드3"],
  "ingredients": [{"name": "재료명", "amount": "2조각 · 30g", "kcal": 숫자, "carb": 숫자, "protein": 숫자, "fat": 숫자}]
}
---JSON_END---`;
    ...
    if (image) {
      ...
      messages = [
        ...
        },
        {
          role: 'user',
          content: [
            { type: 'text', text: `이 음식 사진을 정밀하게 분석해주세요. 음식의 정확한 종류를 먼저 파악하세요 (한식, 일식, 중식, 양식 등 편견 없이). 사진에 보이는 모든 음식과 음료를 빠짐없이 식별하고, 각 재료의 양을 현실적으로 추정해주세요.\n\n중요 - 양 추정 규칙:\n- 실제 보이는 양보다 보수적으로(적게) 추정할 것. 과대 추정은 절대 금지.\n- 잘린 과일 조각은 통째 개수가 아닌 조각 수로 표기 (예: 딸기 6조각 ≠ 딸기 6알, 조각 크기 고려하여 실제 딸기 약 2알 분량으로 환산)\n- 토핑류(그래놀라, 견과류, 시리얼 등)는 그릇 전체가 아닌 실제 토핑된 양만 추정\n- 확신이 없으면 항상 적은 쪽으로 추정\n- 얼음, 물, 소금 등 0kcal 재료는 ingredients에 포함하지 말 것\n- 사진에 여러 음식이 있을 경우 음식당 주요 재료 최대 5개만 표기\n\n${nutritionFields}\n\n${jsonTemplate}` },
