...
---JSON_END---`;
    ...
    if (image) {
      ...
      messages = [
        {
          role: 'system',
          content: 'You are an expert Korean nutritionist with deep knowledge of Korean cuisine and international foods. Carefully examine the food photo: identify every visible dish and ingredient, estimate realistic portion sizes, and calculate accurate nutritional values based on standard Korean serving sizes. If multiple dishes are visible, combine them into one analysis. Output ONLY valid JSON between ---JSON_START--- and ---JSON_END--- markers.',
        },
        {
          role: 'user',
          content: [
            { type: 'text', text: `이 음식 사진을 정밀하게 분석해주세요. 사진에 보이는 모든 음식과 음료를 빠짐없이 식별하고, 각 재료의 양을 현실적으로 추정해주세요.\n\n${nutritionFields}\n\n${jsonTemplate}` },
