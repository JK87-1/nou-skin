  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
        <span onClick={onClick} style={{ fontSize: 14, lineHeight: 1.5, color: 'var(--text-secondary)', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 6, cursor: onClick ? 'pointer' : 'default' }}>{icon} {label}</span>
        <span style={{ fontSize: 16, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'var(--font-display)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          {diff != null && diff !== 0 && (
            <span style={{
      ...
      {description && <div style={{ fontSize: 11, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 6 }}>{description}</div>}
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0' }}>
        ...
        <div style={{ padding: '10px 15px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <h1 style={{ fontSize: 18, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-primary)', margin: 0 }}>
              {data.title}
            </h1>
          </div>
          <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.5, margin: '10px 0 0' }}>{data.hero}</p>
        </div>
      </div>
      ...
      <div style={{ padding: '20px 16px' }}>
        ...
        <div className="card">
          <div style={{ fontSize: 16, lineHeight: 1.3, fontWeight: 500, marginBottom: 14, color: 'var(--text-primary)' }}>결과 해석</div>
        ...
        <DetailAccordion title="측정 원리" onToggle={(open) => setAnyAccordionOpen(p => p + (open ? 1 : -1))}>
          <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.5, whiteSpace: 'pre-line', margin: 0 }}>{data.methodology}</p>
        </DetailAccordion>
        ...
        <DetailAccordion title="과학적 근거" onToggle={(open) => setAnyAccordionOpen(p => p + (open ? 1 : -1))}>
          {data.references.map((ref, i) => (
            <div key={i} style={{
              ...
              <div style={{ padding: '8px 10px', background: 'rgba(255,255,255,0.1)', fontWeight: 500, fontSize: 12, lineHeight: 1.5, color: 'var(--text-secondary)', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>{ref.name}</div>
              <div style={{ padding: '7px 10px', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.5 }}>{ref.description}</div>
              <div style={{ padding: '7px 10px', fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', fontStyle: 'italic', lineHeight: 1.3 }}>{ref.source}</div>
            </div>
          ))}
        </DetailAccordion>

        {/* 4. Disclaimer */}
        <p style={{ fontSize: 10, fontWeight: 500, color: 'var(--text-disabled)', lineHeight: 1.3, textAlign: 'center', margin: '0 0 16px', padding: '0 19px' }}>
