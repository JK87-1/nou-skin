  ...
  return (
    <div style={{ padding: '0 20px 24px' }}>
      ...
      <div style={{ marginBottom: 16, animation: 'swFadeInUp 0.5s ease 0.05s both' }}>
        {/* 날짜 */}
        <div style={{ fontSize: 13, lineHeight: 1.5, fontWeight: 500, color: 'rgba(255,255,255,0.7)', marginBottom: 16, textAlign: 'center' }}>{weather.date} · {weather.condition}</div>

        {/* 온도 + 상태 */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, padding: '0 16px' }}>
          <span style={{ fontSize: 64, fontWeight: 400, color: '#fff', lineHeight: 1.1, letterSpacing: -3 }}>{weather.temp}°</span>
          <WeatherIconFilled emoji={weather.conditionIcon} size={80} />
        </div>
        <div style={{ padding: '0 16px', marginBottom: 40 }}>
          <span style={{ fontSize: 16, lineHeight: 1.3, fontWeight: 500, color: 'rgba(255,255,255,0.85)' }}>{weather.tempMax}° / {weather.tempMin}°</span>
        </div>
      ...
      <div style={{
        ...
        {(() => {
          ...
          return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: '#fff' }}>{weather.humidity}<span style={{ fontSize: 11, lineHeight: 1.3, fontWeight: 500 }}>%</span></div>
                <div style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'rgba(255,255,255,0.7)' }}>습도 · {hi.label}</div>
              </div>
            </div>
          );
        })()}
        ...
        {(() => {
          ...
          return (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: '#fff' }}>{weather.airQuality}<span style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, marginLeft: 2 }}>AQI</span></div>
                <div style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'rgba(255,255,255,0.7)' }}>미세먼지 · {ai.label}</div>
              </div>
            </div>
          );
        })()}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            <div style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: '#fff' }}>{weather.uv}<span style={{ fontSize: 11, lineHeight: 1.3, fontWeight: 500 }}>/10</span></div>
            <div style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'rgba(255,255,255,0.7)' }}>자외선 · {weather.uvLabel}</div>
          </div>
        </div>
      </div>
          ...
          return (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                {fc.map((f, i) => (
                  <div key={i} style={{ flex: 1, textAlign: 'center', fontSize: 11, lineHeight: 1.3, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                {fc.map((f, i) => (
                  <div key={i} style={{ flex: 1, textAlign: 'center', fontSize: 16, lineHeight: 1.3, fontWeight: 500, color: '#fff' }}>
                    {f.temp}°
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                {fc.map((f, i) => (
                  <div key={i} style={{ flex: 1, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
                    ...
                    <span style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'rgba(255,255,255,0.4)' }}>{f.rain ?? 0}%</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {alerts.length > 0 && (
        <div style={{ marginBottom: 16, animation: 'swFadeInUp 0.5s ease 0.15s both', background: 'var(--card-weather-bg)', backdropFilter: 'var(--card-blur)', WebkitBackdropFilter: 'var(--card-blur)', boxShadow: 'var(--card-shadow)', borderRadius: 'var(--card-radius)', padding: '16px 16px 8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-primary)' }}>내 피부 맞춤 알림</span>
            {highCount > 0 && (
              <span style={{
              ...
              <div key={alert.id} style={{
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-primary)' }}>{alert.title}</span>
                      {alert.priority === 'high' && (
                        <div style={{
                    ...
                    <div style={{ fontSize: 11, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-muted)', marginTop: 2 }}>{alert.subtitle}</div>
                  </div>
                  <div style={{ fontSize: 12, lineHeight: 1.5, fontWeight: 500, color: alert.color, marginRight: 4 }}>
                    {alert.matchScore}
                  </div>
                  <span style={{
                    fontSize: 12, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-dim)', transition: 'transform 0.3s',
                ...
                {isOpen && (
                  <div style={{ padding: '0 16px 16px' }}>
                    <div style={{
                      fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: 14,
                    }}>
                      {alert.description}
                    </div>
      ...
      {!skinResult && (
        <div style={{
          ...
          <div style={{ fontSize: 24, lineHeight: 1.3, fontWeight: 500, marginBottom: 8 }}><MicroscopeIcon size={24} /></div>
          <div style={{ fontSize: 13, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 4 }}>피부 측정 후 맞춤 알림을 받아보세요</div>
          <div style={{ fontSize: 11, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-muted)' }}>분석 결과를 기반으로 오늘 환경에 맞는 케어 팁을 드려요</div>
        </div>
      )}
      ...
      {weather.weekForecast && weather.weekForecast.length > 0 && (
        <div style={{ marginBottom: 16, animation: 'swFadeInUp 0.5s ease 0.2s both' }}>
          <div style={{
            ...
            <div style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 10 }}>
              5일간 피부 환경 예보
            </div>
            {weather.weekForecast.map((day, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 13, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-muted)', width: 22, flexShrink: 0 }}>{day.day}</span>
                  ...
                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 12, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-dim)', width: 28, textAlign: 'right' }}>{day.min}°</span>
                    <div style={{
                    ...
                    <span style={{ fontSize: 12, lineHeight: 1.5, color: 'var(--text-secondary)', fontWeight: 500, width: 28 }}>{day.max}°</span>
                  </div>
                  <span style={{
                    fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: '#fff',
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-primary)' }}>{seasonal.title}</div>
          <div style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-muted)', marginTop: 2 }}>{seasonal.season} 시즌 가이드</div>
        </div>
        <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: 14 }}>
          {seasonal.content}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {seasonal.keyPoints.map((kp, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: 18, lineHeight: 1.3, fontWeight: 500, marginBottom: 4, display: 'flex', justifyContent: 'center' }}>{kp.icon}</div>
              <div style={{ fontSize: 11, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 2 }}>{kp.label}</div>
              <div style={{ fontSize: 10, fontWeight: 500, color: 'var(--text-dim)', lineHeight: 1.3 }}>{kp.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {notifications.length > 0 && (() => {
        ...
        <div style={{ marginBottom: 8, animation: 'swFadeInUp 0.5s ease 0.4s both', background: 'var(--card-weather-bg)', backdropFilter: 'var(--card-blur)', WebkitBackdropFilter: 'var(--card-blur)', boxShadow: 'var(--card-shadow)', borderRadius: 'var(--card-radius)', padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ fontSize: 14, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-primary)' }}>오늘 예정된 알림</div>
            {weatherEnabled ? (
              <span style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: '#6598EF', background: 'rgba(var(--accent-rgb),0.1)', padding: '3px 8px', borderRadius: 8 }}>알림 ON</span>
            ) : (
              <span style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-dim)', background: 'rgba(255,255,255,0.08)', padding: '3px 8px', borderRadius: 8 }}>알림 OFF</span>
            )}
          </div>
          {!weatherEnabled && (
            <div style={{ padding: '10px 14px', borderRadius: 12, marginBottom: 10, background: 'rgba(var(--accent-rgb),0.08)', border: 'none', fontSize: 11, fontWeight: 500, color: 'var(--text-secondary)', lineHeight: 1.3 }}>
          ...
          {notifications.map((n, i) => {
            ...
            <div key={i} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, lineHeight: 1.5, fontWeight: 500, color: 'var(--text-secondary)', textDecoration: isPast ? 'line-through' : 'none' }}>{n.title}</div>
                <div style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: 'var(--text-dim)', marginTop: 1 }}>{n.body}</div>
              </div>
              <span style={{ fontSize: 10, lineHeight: 1.3, fontWeight: 500, color: isPast ? 'var(--text-dim)' : 'var(--text-muted)', flexShrink: 0 }}>{isPast ? '완료' : n.time}</span>
            </div>
            );
          })}
        </div>
