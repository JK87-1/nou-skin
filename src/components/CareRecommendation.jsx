...
// ===== Tabler Icons (Filled) =====
const TI = ({ children, size = 18 }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="#6598ef" stroke="none" fillRule="evenodd">{children}</svg>;
...
const IconBulb = ({ size }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="#6598ef" stroke="none">
    <path d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2zm0 13a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm0-8a1 1 0 0 0-1 1v4a1 1 0 0 0 2 0V8a1 1 0 0 0-1-1z" />
  </svg>
);
...
function StepBlock({ stepDef, order }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{
        ...
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)' }}>{stepDef.products.length}개</span>
      </div>
...
function RoutineColumn({ title, subtitle, steps, totalProducts, hideHeader = false }) {
      ...
      {!hideHeader ? (
        <div style={{ padding: '0 4px', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-primary)', letterSpacing: -0.2 }}>{title}</span>
            <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)' }}>{totalProducts}개</span>
          </div>
          {subtitle && (
    ...
    <div style={{ marginTop: 8, marginBottom: 24, background: 'var(--card-bg)', backdropFilter: 'var(--card-blur)', WebkitBackdropFilter: 'var(--card-blur)', borderRadius: 'var(--card-radius)', boxShadow: 'var(--card-shadow)', padding: '20px 14px' }}>
      ...
      {!hideHeader && (
        <div style={{ padding: '0 4px', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', lineHeight: 1.55 }}>
            등록한 화장품을 표준 순서대로 정리했어요.
            {result?.overallScore != null && ` 종합 ${result.overallScore}점 기준 매칭도 높은 순.`}
          </div>
          <div style={{ fontSize: 12, color: 'var(--accent-primary)', lineHeight: 1.55, marginTop: 6, fontWeight: 500 }}>
      ...
      {(() => {
        ...
        return (
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 3 }}>{pick.title}</div>
              <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', lineHeight: 1.55 }}>{pick.advice}</div>
            </div>
          </div>
        );
      })()}
...
function RoutineTabs({ morningSteps, nightSteps, morningStats, nightStats }) {
  ...
  const tabs = [
    { key: 'morning', label: '아침', count: morningStats.total },
    { key: 'night',   label: '저녁', count: nightStats.total },
  ];
          ...
          return (
            <button
              ...
              style={{
                ...
                fontSize: 14, fontWeight: 500, fontWeight: active ? 600 : 500,
                letterSpacing: -0.2,
