...
function SheetHandle({ isLight }) {
  return <div style={{ width: 36, height: 4, borderRadius: 2, margin: '0 auto 20px', background: isLight ? '#ddd' : 'rgba(255,255,255,0.15)' }} />;
}
...
function CategorySelector({ value, onChange, accent, isLight }) {
  return (
      ...
      {CAT_KEYS.map(key => {
        ...
        return (
          <button key={key} onClick={() => onChange(key)} style={{
            border: active ? `1.5px solid ${cat.color}` : isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.08)',
            background: active ? `${cat.color}18` : isLight ? '#F7F8FA' : 'rgba(255,255,255,0.04)',
            borderRadius: 12, padding: '10px 4px', cursor: 'pointer',
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
          }}>
            <span style={{ fontSize: 18 }}>{cat.emoji}</span>
            <span style={{ fontSize: 11, fontWeight: active ? 600 : 400, color: active ? cat.color : (isLight ? '#6B7684' : '#8888a0') }}>{key}</span>
          </button>
        );
      })}
    </div>
...
function TimeSlotSelector({ value, onChange, accent, isLight }) {
  const opts = [
      ...
      {opts.map(o => {
        ...
        return (
          <button key={o.key} onClick={() => onChange(o.key)} style={{
            ...
            border: active ? `1.5px solid ${accent}` : isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.08)',
...
function AddProductSheet({ onClose, onPhoto, onManual, accent, isLight }) {
  return (
    <SheetOverlay onClose={onClose}>
      <div style={{ background: isLight ? '#FFFFFF' : '#1a1a24', padding: '24px 20px 40px' }}>
        <SheetHandle isLight={isLight} />
        <div style={{ fontSize: 18, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 20 }}>제품 등록</div>
        {[
          ...
          <div key={i} onClick={opt.action} style={{
            ...
            background: isLight ? '#F7F8FA' : 'rgba(255,255,255,0.04)',
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 600, color: isLight ? '#191F28' : '#f0f0f5' }}>{opt.label}</div>
              <div style={{ fontSize: 12, color: isLight ? '#8B95A1' : '#8888a0', marginTop: 2 }}>{opt.desc}</div>
            </div>
          </div>
        ))}
      </div>
...
function PhotoRegistrationFlow({ onClose, onSave, saving, accent, isLight }) {
  ...
  return (
    <SheetOverlay onClose={onClose}>
      <div style={{ background: isLight ? '#FFFFFF' : '#1a1a24', padding: '24px 20px calc(40px + env(safe-area-inset-bottom, 0px))' }}>
        <SheetHandle isLight={isLight} />

        {/* 촬영 단계 */}
        {step === 'capture' && (
          <>
            <div style={{ fontSize: 18, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 8 }}>제품 사진 촬영</div>
            <p style={{ fontSize: 13, color: isLight ? '#8B95A1' : '#8888a0', marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {[
                ...
                <button key={i} onClick={() => btn.ref.current?.click()} style={{
                  ...
                  border: isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.08)',
                  background: isLight ? '#F7F8FA' : 'rgba(255,255,255,0.04)',
        ...
        {step === 'loading' && loading && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 8 }}>
              AI가 제품을 분석중이에요...
            </div>
            <div style={{
            ...
            <button onClick={() => { setLoading(false); setError('인식을 취소했어요. 정보를 직접 입력해주세요.'); setStep('confirm'); }} style={{
              padding: '8px 20px', borderRadius: 10, border: isLight ? '1px solid #ddd' : '1px solid rgba(255,255,255,0.1)',
              background: 'transparent', color: isLight ? '#6B7684' : '#8888a0', fontSize: 13, cursor: 'pointer',
            }}>직접 입력하기</button>
        ...
        {step === 'confirm' && (
          <>
            <div style={{ fontSize: 18, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 16 }}>제품 정보 확인</div>
            ...
            {form.ingredients.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: isLight ? '#6B7684' : '#8888a0', marginBottom: 6 }}>인식된 성분</div>
            ...
            {[
              ...
              <div key={f.key} style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: isLight ? '#6B7684' : '#8888a0', marginBottom: 6 }}>{f.label}</div>
                <input
                  type="text"
                  autoComplete="off"
                  value={form[f.key]}
                  ...
                  style={{
                    ...
                    border: isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.1)',
                    background: isLight ? '#F7F8FA' : 'rgba(255,255,255,0.04)',
                    color: isLight ? '#191F28' : '#f0f0f5',
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: isLight ? '#6B7684' : '#8888a0', marginBottom: 6 }}>카테고리</div>
              <CategorySelector value={form.category} onChange={v => setForm(p => ({ ...p, category: v }))} accent={accent} isLight={isLight} />
            </div>

            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: isLight ? '#6B7684' : '#8888a0', marginBottom: 6 }}>사용 시간대</div>
              <TimeSlotSelector value={form.timeSlot} onChange={v => setForm(p => ({ ...p, timeSlot: v }))} accent={accent} isLight={isLight} />
            </div>
            ...
            <button type="button" onClick={handleSave} disabled={!form.brand.trim() || !form.name.trim() || saving} style={{
              ...
              background: (!form.brand.trim() || !form.name.trim() || saving) ? (isLight ? '#eee' : '#333') : accent,
              color: (!form.brand.trim() || !form.name.trim() || saving) ? (isLight ? '#aaa' : '#666') : '#fff',
...
function ManualRegistrationForm({ onClose, onSave, saving, accent, isLight }) {
  ...
  return (
    <SheetOverlay onClose={onClose}>
      <div style={{ background: isLight ? '#FFFFFF' : '#1a1a24', padding: '24px 20px calc(40px + env(safe-area-inset-bottom, 0px))' }}>
        <SheetHandle isLight={isLight} />
        <div style={{ fontSize: 18, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 20 }}>제품 직접 입력</div>
        ...
        <button type="button" onClick={handleSave} disabled={!canSave} style={{
          ...
          background: canSave ? accent : (isLight ? '#eee' : '#333'),
          color: canSave ? '#fff' : (isLight ? '#aaa' : '#666'),
          fontSize: 15, fontWeight: 700, position: 'relative', zIndex: 1,
        }}>{saving ? '제품 이미지 검색 중...' : '등록하기'}</button>
      </div>
    </SheetOverlay>
...
function ProductDetailSheet({ product, onClose, onDelete, onEdit, accent, isLight }) {
        ...
        {!editing ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              ...
              <div>
                <div style={{ fontSize: 11, color: isLight ? '#8B95A1' : '#8888a0' }}>{product.brand}</div>
                <div style={{ fontSize: 17, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5' }}>{product.name}</div>
                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                  ...
                  <span style={{ fontSize: 10, color: isLight ? '#ADB5BD' : '#666' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
              <div style={{ flex: 1, borderRadius: 12, padding: '14px', textAlign: 'center', background: isLight ? '#F7F8FA' : 'rgba(255,255,255,0.03)' }}>
                <div style={{ fontSize: 10, color: isLight ? '#8B95A1' : '#8888a0', marginBottom: 4 }}>사용 기간</div>
                <div style={{ fontSize: 20, fontWeight: 700, color: accent, fontFamily: 'var(--font-display)' }}>{days}일</div>
              </div>
              <div style={{ flex: 1, borderRadius: 12, padding: '14px', textAlign: 'center', background: isLight ? '#F7F8FA' : 'rgba(255,255,255,0.03)' }}>
                <div style={{ fontSize: 10, color: isLight ? '#8B95A1' : '#8888a0', marginBottom: 4 }}>시작일</div>
                <div style={{ fontSize: 14, fontWeight: 600, color: isLight ? '#191F28' : '#e0e0e8' }}>{product.startDate}</div>
              </div>
            </div>

            {product.ingredients?.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: isLight ? '#6B7684' : '#8888a0', marginBottom: 6 }}>핵심 성분</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            {!confirmDelete ? (
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setEditing(true)} style={{
                  ...
                  border: isLight ? `1px solid ${accent}30` : `1px solid ${accent}40`,
                  background: 'transparent', color: accent, fontSize: 14, fontWeight: 600, cursor: 'pointer',
                }}>수정</button>
                <button onClick={() => setConfirmDelete(true)} style={{
                  flex: 1, padding: '12px 0', borderRadius: 12,
                  border: isLight ? '1px solid #fee' : '1px solid rgba(239,68,68,0.2)',
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setConfirmDelete(false)} style={{
                  flex: 1, padding: '12px 0', borderRadius: 12, border: isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.1)',
                  background: 'transparent', color: isLight ? '#6B7684' : '#8888a0', fontSize: 14, fontWeight: 600, cursor: 'pointer',
                }}>취소</button>
          ...
          <>
            <div style={{ fontSize: 18, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 20 }}>제품 정보 수정</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setEditing(false)} style={{
                ...
                border: isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.1)',
                background: 'transparent', color: isLight ? '#6B7684' : '#8888a0',
                fontSize: 15, fontWeight: 600, cursor: 'pointer',
              }}>취소</button>
              <button onClick={handleSaveEdit} disabled={!form.brand.trim() || !form.name.trim()} style={{
                flex: 1, padding: '14px 0', borderRadius: 14, border: 'none',
                background: (form.brand.trim() && form.name.trim()) ? accent : (isLight ? '#eee' : '#333'),
                color: (form.brand.trim() && form.name.trim()) ? '#fff' : (isLight ? '#aaa' : '#666'),
...
export default function RoutineTracker({ colorMode, themeColors, onBack }) {
    ...
    <div style={{ minHeight: '100vh', paddingBottom: 100, animation: 'breatheIn 0.5s ease both' }}>
      ...
      <div style={{ padding: '52px 20px 0', display: 'flex', alignItems: 'center', gap: 14 }}>
        <button onClick={onBack} style={{
          ...
          background: isLight ? '#F2F3F5' : 'rgba(255,255,255,0.06)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M15 18l-6-6 6-6" stroke={isLight ? '#191F28' : '#f0f0f5'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, color: isLight ? '#191F28' : '#f0f0f5', fontFamily: 'var(--font-display)' }}>스킨케어 트래커</h1>
          <p style={{ fontSize: 12, color: isLight ? '#8B95A1' : '#8888a0', margin: '2px 0 0' }}>제품 등록 · 루틴 관리 · 효과 분석</p>
        </div>
      </div>
        ...
        {sections.map(s => {
          ...
          return (
            <button key={s.key} onClick={() => setSection(s.key)} style={{
              border: active ? `1px solid ${accent}40` : isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.08)',
              background: active ? `${accent}18` : isLight ? '#F7F8FA' : 'rgba(255,255,255,0.04)',
              color: active ? accent : (isLight ? '#6B7684' : '#8888a0'),
      ...
      {section === 'products' && (
        <div style={{ padding: '20px 20px 0', animation: 'fadeUp 0.3s ease-out' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div>
              <span style={{ fontSize: 16, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5' }}>등록된 제품</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 12, alignItems: 'stretch' }}>
            {products.map((p, i) => {
              ...
              return (
                <div key={p.id} onClick={() => setSelectedProduct(p)} className="card" style={{
                  ...
                  <div style={{ fontSize: 11, color: isLight ? '#8B95A1' : '#8888a0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.brand}</div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: isLight ? '#191F28' : '#f0f0f5', marginTop: 2, lineHeight: 1.4, minHeight: '2.8em', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{p.name}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 'auto', paddingTop: 10 }}>
                    ...
                    <span style={{ fontSize: 10, color: isLight ? '#ADB5BD' : '#666', flexShrink: 0 }}>
                      {p.timeSlot === 'both' ? '아침·저녁' : p.timeSlot === 'morning' ? '아침' : '저녁'}
                    </span>
                  </div>
                  <div style={{ fontSize: 11, color: isLight ? '#ADB5BD' : '#666', marginTop: 6 }}>{days}일째 사용중</div>
                </div>
              );
            })}
            ...
            <div onClick={() => setShowAddSheet(true)} style={{
              ...
              border: isLight ? '2px dashed #ddd' : '2px dashed rgba(255,255,255,0.1)',
      ...
      {section === 'routine' && (
        <div style={{ padding: '20px 20px 0', animation: 'fadeUp 0.3s ease-out' }}>
          ...
          {modeProducts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 4 }}>
                {routineMode === 'morning' ? '아침' : '저녁'} 루틴 제품이 없어요
              </div>
              <div style={{ fontSize: 13, color: isLight ? '#8B95A1' : '#8888a0', marginBottom: 16 }}>제품을 등록하고 루틴을 관리해보세요</div>
            ...
            <>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: isLight ? '#191F28' : '#e0e0e8' }}>오늘 진행률</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: accent }}>{progress.done}/{progress.total} 완료</span>
                </div>
                <div style={{ height: 8, borderRadius: 4, overflow: 'hidden', background: isLight ? '#F2F3F5' : 'rgba(255,255,255,0.06)' }}>
                  <div style={{
                ...
                {modeProducts.map((p, idx) => {
                  ...
                  return (
                    <div key={p.id} style={{
                      ...
                      borderBottom: idx < modeProducts.length - 1 ? (isLight ? '1px solid #F2F3F5' : '1px solid rgba(255,255,255,0.04)') : 'none',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 500, color: isLight ? '#191F28' : '#f0f0f5', textDecoration: isChecked ? 'line-through' : 'none' }}>{p.name}</div>
                        <div style={{ fontSize: 11, color: isLight ? '#ADB5BD' : '#666', marginTop: 1 }}>{p.brand}</div>
                      </div>
          ...
          <div className="card" style={{ padding: '16px 18px' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: isLight ? '#191F28' : '#e0e0e8', marginBottom: 14 }}>주간 루틴 현황</div>
              ...
              {weekly.map((day) => (
                <div key={day.dayLabel} style={{ textAlign: 'center' }}>
                  <div style={{
                    ...
                    background: day.completed ? accent : day.isToday ? `${accent}20` : day.partial ? `${accent}10` : isLight ? '#F2F3F5' : 'rgba(255,255,255,0.04)',
                  ...
                  <span style={{ fontSize: 10, fontWeight: day.isToday ? 600 : 400, color: day.isToday ? accent : (isLight ? '#8B95A1' : '#8888a0') }}>{day.dayLabel}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ padding: '20px 20px 0', animation: 'fadeUp 0.3s ease-out' }}>
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 16, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5' }}>제품별 효과 분석</span>
              <span style={{ fontSize: 10, fontWeight: 600, color: accent, background: `${accent}15`, borderRadius: 8, padding: '3px 8px' }}>데이터 기반</span>
            </div>
            <p style={{ fontSize: 12, color: isLight ? '#8B95A1' : '#8888a0', margin: '4px 0 0' }}>
          ...
          {products.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 4 }}>등록된 제품이 없어요</div>
              <div style={{ fontSize: 13, color: isLight ? '#8B95A1' : '#8888a0' }}>제품을 등록하면 피부 변화와의 상관관계를 분석해드려요</div>
            </div>
          ) : analyses.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: isLight ? '#191F28' : '#f0f0f5', marginBottom: 4 }}>분석에 필요한 데이터가 부족해요</div>
              <div style={{ fontSize: 13, color: isLight ? '#8B95A1' : '#8888a0' }}>피부 측정을 2회 이상 하면 분석이 시작돼요</div>
            </div>
          ) : (
            analyses.map((a, idx) => {
              ...
              return (
                <div key={a.productId} className="card" style={{ padding: 20, marginBottom: 16, animation: `breatheIn 0.5s ease ${idx * 0.15}s both` }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: isLight ? '#191F28' : '#f0f0f5' }}>{a.productName}</div>
                      <div style={{ fontSize: 11, color: isLight ? '#8B95A1' : '#8888a0' }}>{a.brand}</div>
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 600, color: accent, background: `${accent}15`, borderRadius: 10, padding: '4px 10px' }}>{a.days}일</span>
                  </div>

                  {a.chart && (
                    <div style={{ borderRadius: 12, overflow: 'hidden', marginBottom: 16, background: isLight ? '#FAFBFC' : 'rgba(255,255,255,0.02)', padding: '12px 8px 4px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                        {a.metrics.map((m, mi) => (
                          <div key={mi} style={{
                            ...
                            <span style={{ fontSize: 12, color: isLight ? '#6B7684' : '#aaa' }}>{m.label}</span>
                      ...
                      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
                        {[
                          ...
                          <div key={bi} style={{
                            ...
                            background: isLight ? '#F7F8FA' : 'rgba(255,255,255,0.03)',
                            border: isLight ? '1px solid #eee' : '1px solid rgba(255,255,255,0.06)',
                          }}>
                            <div style={{ fontSize: 10, color: isLight ? '#8B95A1' : '#8888a0', marginBottom: 4 }}>{box.label}</div>
                            <div style={{ fontSize: 22, fontWeight: 700, color: bi === 1 ? accent : (isLight ? '#191F28' : '#e0e0e8'), fontFamily: 'var(--font-display)' }}>{box.value}</div>
                          </div>
                        ))}
                      </div>
                    </>
                  ...
                  <div style={{
                    ...
                    background: isLight ? 'rgba(124,92,252,0.06)' : 'rgba(240,144,112,0.06)',
                    border: isLight ? '1px solid rgba(124,92,252,0.1)' : '1px solid rgba(240,144,112,0.1)',
                  }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                      <span style={{ fontSize: 18, flexShrink: 0 }}>🤖</span>
                      <p style={{ fontSize: 13, lineHeight: 1.6, margin: 0, color: isLight ? '#3D4551' : '#e0e0e8' }}>{a.insight}</p>
                    </div>
                  </div>
                </div>
              );
