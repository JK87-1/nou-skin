          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                  <span style={{ fontSize: 10, fontWeight: 500, color: cat.color, background: `${cat.color}15`, borderRadius: 6, padding: '2px 7px' }}>{product.category}</span>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
              <div style={{ flex: 1, borderRadius: 'var(--card-radius)', padding: '14px', textAlign: 'center', background: 'var(--card-bg)', backdropFilter: 'var(--card-blur)', WebkitBackdropFilter: 'var(--card-blur)', border: 'none', boxShadow: 'var(--card-shadow)' }}>
                <div style={{ fontSize: 10, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 4 }}>사용 기간</div>
                <div style={{ fontSize: 18, fontWeight: 500, color: accent, fontFamily: 'var(--font-display)' }}>{days}일</div>
              </div>
              <div style={{ flex: 1, borderRadius: 'var(--card-radius)', padding: '14px', textAlign: 'center', background: 'var(--card-bg)', backdropFilter: 'var(--card-blur)', WebkitBackdropFilter: 'var(--card-blur)', border: 'none', boxShadow: 'var(--card-shadow)' }}>
                <div style={{ fontSize: 10, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 4 }}>시작일</div>
                <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)' }}>{product.startDate}</div>
              </div>
            </div>

            {product.ingredients?.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--tag-color)', marginBottom: 6 }}>핵심 성분</div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {product.ingredients.map((ing, i) => (
                    <span key={i} style={{ fontSize: 11, fontWeight: 500, color: accent, background: `${accent}12`, borderRadius: 8, padding: '4px 10px' }}>{ing}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {!confirmDelete ? (
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setEditing(true)} style={{
                  ...
                  background: 'transparent', color: accent, fontSize: 14, fontWeight: 500, cursor: 'pointer',
                }}>수정</button>
                <button onClick={() => setConfirmDelete(true)} style={{
                  flex: 1, padding: '12px 0', borderRadius: 12,
                  border: 'none',
                  background: 'transparent', color: '#ef4444', fontSize: 14, fontWeight: 500, cursor: 'pointer',
                }}>삭제</button>
              </div>
            ) : (
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setConfirmDelete(false)} style={{
                  ...
                  background: 'transparent', color: 'var(--tag-color)', fontSize: 14, fontWeight: 500, cursor: 'pointer',
                }}>취소</button>
                <button onClick={() => { hapticWarning(); onDelete(product.id); }} style={{
                  flex: 1, padding: '12px 0', borderRadius: 12, border: 'none',
                  background: '#ef4444', color: '#fff', fontSize: 14, fontWeight: 500, cursor: 'pointer',
                }}>삭제 확인</button>
              </div>
            )}
          </>
    ...
    <div style={{ minHeight: '100dvh', paddingBottom: 100, animation: 'breatheIn 0.5s ease both' }}>
          ...
          <div style={{
            ...
            {products.length === 0 ? (
              <div style={{ padding: '32px 16px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-dim)' }}>위 '제품 등록' 버튼으로 추가해보세요</div>
              </div>
            ) : (
              ...
              {products.map((p, pIdx) => {
                ...
                >
                  <div onClick={() => setSelectedProduct(p)} style={{
                    ...
                    <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.brand}</div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginTop: 2, lineHeight: 1.4, minHeight: '2.8em', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{p.name}</div>
                  ...
                  <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{
                          fontSize: 10, fontWeight: 500, color: 'var(--accent-primary)',
        ...
        <div>
          ...
          {modeProducts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 16 }}>제품을 등록하고 루틴을 관리해보세요</div>
            ...
            <>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>오늘 진행률</span>
                  <span style={{ fontSize: 13, fontWeight: 500, color: accent }}>{progress.done}/{progress.total} 완료</span>
                </div>
                  ...
                  return (
                    <div key={p.id} style={{
                      ...
                      {p.imageThumb ? (
                        ...
                        <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: `${cat.color}12`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 500 }}>{cat.emoji}</div>
                      )}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-primary)', textDecoration: isChecked ? 'line-through' : 'none' }}>{p.name}</div>
                        <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-dim)', marginTop: 1 }}>{p.brand}</div>
                      </div>
                      <span style={{ fontSize: 10, fontWeight: 500, color: cat.color, background: `${cat.color}12`, borderRadius: 6, padding: '2px 7px', flexShrink: 0 }}>{p.category}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="card" style={{ padding: '16px 18px' }}>
            <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-secondary)', marginBottom: 14 }}>주간 루틴 현황</div>
            <div style={{ display: 'flex', justifyContent: 'space-around' }}>
              {weekly.map((day) => (
                <div key={day.dayLabel} style={{ textAlign: 'center' }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 500, fontWeight: day.isToday ? 600 : 400, color: day.isToday ? accent : ('var(--text-muted)') }}>{day.dayLabel}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {section === 'analysis' && (
        <div style={{ padding: '20px 20px 0', animation: 'fadeUp 0.3s ease-out' }}>
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 16, fontWeight: 500, color: 'var(--text-primary)' }}>제품별 효과 분석</span>
