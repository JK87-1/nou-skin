    ...
    <div className="ux-stagger" style={{ minHeight: '100dvh', paddingBottom: 64 }}>
      ...
      <div style={{ margin: '0 16px 12px', padding: '0 2px' }}>
            ...
            {[
              ...
              return (
                <div key={t.key} onClick={() => setContentMode(t.key)} style={{
                  ...
                  <div style={{ fontSize: 11, fontWeight: 400, marginTop: 4, color: itemColor, fontWeight: active ? 500 : 400, transition: 'color 0.2s' }}>{t.label}</div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ paddingTop: 4 }}>
          <div style={{ fontSize: 14, fontWeight: 400, color: '#0f0f0f' }}>{profile.nickname || 'user'}</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 6, marginBottom: 20 }}>
            <div onClick={() => setBioModal(true)} style={{ fontSize: 12, fontWeight: 400, color: 'rgba(15,15,15,0.4)', cursor: 'pointer' }}>
              {profile.bio || '자기소개'}
            </div>
        ...
        <div>
          {recentPhotos.length === 0 ? (
            <div style={{ padding: '32px 16px', textAlign: 'center' }}>
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 4 }}>측정한 셀카가 여기에 쌓여요</div>
                <div style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-muted)', marginBottom: 12 }}>첫 측정을 시작해보세요</div>
                <button onClick={() => onMeasure?.()} style={{ background: 'var(--accent-primary, var(--accent-primary))', color: '#fff', fontSize: 11, fontWeight: 400, padding: '8px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>첫 측정 시작</button>
            </div>
          ) : (<>
            {/* 그리드 뷰 */}
            {albumView === 'grid' && (
                  ...
                  return (
                    <div key={p.record.id || p.date} role="button" aria-label={`${shortDate} 측정 결과 ${p.record.overallScore ?? ''}점`} onClick={() => onViewRecord?.(p.record, p.thumb)} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 11, fontWeight: 400, color: 'rgba(0,0,0,0.4)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6, marginTop: 10 }}>
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                      <button key={n} onClick={() => setListPage(n)} style={{
                        ...
                        fontSize: 12, fontWeight: 400, fontWeight: n === listPage ? 600 : 400,
      ...
      {contentMode === 'history' && (
        <div style={{ padding: '32px 16px', textAlign: 'center' }}>
          <div style={{ fontSize: 40, fontWeight: 300, opacity: 0.3, marginBottom: 8 }}></div>
          <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 6 }}>곧 만나요</div>
          <div style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-muted)', lineHeight: 1.5 }}>측정 기록을 한 자리에 정돈해드릴게요</div>
        </div>
      )}

      {contentMode === 'record' && (
        <div style={{ padding: '16px 16px 0' }}>
          <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-muted)', textAlign: 'center', padding: '20px 0' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '0 16px 16px' }}>
          <textarea value={text} onChange={e => setText(e.target.value.slice(0, 50))} placeholder="입력해주세요"
            style={{ width: '100%', minHeight: 80, background: '#ffffff', border: 'none', borderRadius: 12, padding: '10px 14px', fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit', resize: 'none', boxSizing: 'border-box' }} />
          <div style={{ textAlign: 'right', fontSize: 10, fontWeight: 500, color: 'var(--text-muted)', marginTop: 4 }}>{text.length}/50</div>
          <button onClick={() => onSave(text)} style={{ width: '100%', marginTop: 12, padding: 14, borderRadius: 10, border: 'none', background: 'var(--accent-primary, var(--accent-primary))', color: '#fff', fontSize: 13, fontWeight: 400, cursor: 'pointer', fontFamily: 'inherit' }}>두기</button>
        </div>
      </div>
    </>
  );
...
function SettingsModal({ profile, update, onClose, showToast, colorMode, setColorMode, colorSkin, setColorSkin }) {
  ...
  const SettingsRow = ({ icon, label, right, onTap }) => (
    <div onClick={onTap} style={{
      ...
      {right && <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-muted)' }}>{right}</span>}
  ...
  const SectionHeader = ({ label }) => (
    <div style={{ padding: '18px 28px 6px', fontSize: 12, fontWeight: 400, color: 'rgba(0,0,0,0.4)', letterSpacing: 0.8 }}>{label}</div>
  );
  ...
  return (
    ...
    <div style={{
      ...
      <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
        ...
        <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>설정</span>
      </div>
        ...
        {showTimePicker === 'page' && (
          ...
          <div style={{
            ...
            <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
              ...
              <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>알림</span>
            </div>
            ...
            <div style={{ padding: '50px 40px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ padding: '6px 0', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>진단 리마인더</div>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  ...
                  {reminderEnabled && (
                    <div onClick={() => setShowTimePicker('reminder')} style={{
                      ...
                      <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)' }}>알림 시간</span>
                      <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--accent-primary, var(--accent-primary))' }}>{formatPushTime(reminderTime)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ padding: '6px 0', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>뷰티 팁</div>
                  ...
                  {tipEnabled && (
                    <div onClick={() => setShowTimePicker('tip')} style={{
                      ...
                      <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)' }}>알림 시간</span>
                      <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--accent-primary, var(--accent-primary))' }}>{formatPushTime(tipTime)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div style={{ padding: '6px 0', display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>피부 날씨 알림</div>
        ...
        {showTimePicker === 'style' && (
          ...
          <div style={{
            ...
            <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
              ...
              <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>스타일</span>
            </div>

            {/* 테마 프리뷰 카드 */}
            <div style={{ padding: '40px 28px 0' }}>
              <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 14 }}>테마</div>
                  ...
                  return (
                    <div key={t.key} onClick={() => setColorSkin(t.key)} style={{
                      ...
                      <div style={{ textAlign: 'center', marginTop: 8 }}>
                        <div style={{ fontSize: 13, fontWeight: 400, fontWeight: selected ? 600 : 500, color: selected ? 'var(--text-primary)' : 'var(--text-muted)' }}>{t.label}</div>
                      </div>
                      {selected && (
      ...
      {legalPage && (
        <div style={{
          ...
          <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '25px 20px 40px', WebkitOverflowScrolling: 'touch' }}>
            {(() => {
              ...
              while (i < lines.length) {
                ...
                if (line.startsWith('|')) {
                  ...
                  elements.push(
                    <div key={key} style={{ overflowX: 'auto', margin: '10px 0', borderRadius: 5, border: 'none', WebkitOverflowScrolling: 'touch' }}>
                      <table style={{ width: '100%', minWidth: 360, borderCollapse: 'collapse', fontSize: 12, fontWeight: 400, tableLayout: 'auto' }}>
                        <thead>
                ...
                if (line.startsWith('- ')) { const bp = line.slice(2).split(/(\*\*[^*]+\*\*)/g); elements.push(<div key={key} style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', margin: '3px 0', paddingLeft: 14, lineHeight: 1.6 }}>• {bp.map((p, j) => p.startsWith('**') && p.endsWith('**') ? <strong key={j} style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{p.slice(2, -2)}</strong> : p)}</div>); i++; continue; }
                if (/^\d+\. /.test(line)) { elements.push(<div key={key} style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', margin: '3px 0', paddingLeft: 14, lineHeight: 1.6 }}>{line}</div>); i++; continue; }
                if (line === '') { elements.push(<div key={key} style={{ height: 6 }} />); i++; continue; }
                const parts = line.split(/(\*\*[^*]+\*\*)/g);
                elements.push(
                  <p key={key} style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.75, margin: 0 }}>
                    {parts.map((p, j) =>
      ...
      {faqOpen && (
        ...
        <div style={{
          ...
          <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>문의하기</span>
          </div>
            ...
            {INQUIRY_FAQ.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span style={{ flex: 1, fontSize: 14, fontWeight: 400, color: 'var(--text-primary)', lineHeight: 1.4 }}>
                      {idx + 1}. {item.q}
                    </span>
                    <svg
                  ...
                  {isOpen && (
                    <div style={{
                      padding: '0 16px 16px',
                      fontSize: 14, fontWeight: 400, color: 'var(--text-secondary)', lineHeight: 1.7,
                      whiteSpace: 'pre-line',
                    }}>
                      {item.a}
                    </div>
      ...
      {editingProfile && (
        <div style={{
          ...
          <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>프로필</span>
          </div>
          ...
          <div style={{ textAlign: 'center', padding: '50px 0 30px' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              <div style={{
                ...
                {profile.profileImage
                  ? <img src={profile.profileImage} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <span style={{ fontSize: 40, fontWeight: 300, color: 'var(--text-muted)', letterSpacing: -0.6 }}>{(profile.nickname || '?')[0].toUpperCase()}</span>
                }
              </div>
          ...
          <div style={{ margin: '0 40px', display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ padding: '6px 0' }}>
              <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 8 }}>이름</div>
              <input
                type="text"
                ...
                style={{
                  ...
                  padding: '12px 14px', fontSize: 13, fontWeight: 400, color: 'var(--text-primary)',
            ...
            {/* 생년월일 */}
            <div style={{ padding: '6px 0' }}>
              <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 8 }}>생년월일</div>
              <input
                type="tel"
                inputMode="numeric"
                value={(() => {
            ...
            {/* 성별 */}
            <div style={{ padding: '6px 0' }}>
              <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 8 }}>성별</div>
            ...
            {/* 자기소개 */}
            <div style={{ padding: '6px 0' }}>
              <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 8 }}>자기소개</div>
              <div style={{ position: 'relative' }}>
                <textarea
                  ...
                  style={{
                    ...
                    borderRadius: 12, padding: '12px 14px', fontSize: 12, fontWeight: 400, color: 'var(--text-primary)',
          ...
          <div style={{ padding: '16px 40px calc(16px + env(safe-area-inset-bottom,0px))' }}>
            <button onClick={() => { showToast('저장되었어요'); setEditingProfile(false); }} style={{
              ...
              background: 'var(--accent-bg, rgba(var(--accent-rgb),0.1))', color: 'var(--accent-primary, var(--accent-primary))', fontSize: 14, fontWeight: 400,
      ...
      {editingSkin && (
        <div style={{
          ...
          <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>피부 타입</span>
          </div>
            ...
            {[
              ...
              return (
                <div key={opt.key} onClick={() => update('skinType', opt.key)} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>{opt.key}</div>
                    <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.6 }}>{opt.desc}</div>
                  </div>
                </div>
              );
            })}
            ...
            <div onClick={() => update('isSensitive', !profile.isSensitive)} style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>민감성</div>
                ...
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.6 }}>쉽게 자극받고 환절기에 트러블이 잘 나요</div>
              </div>
            </div>
          </div>
      ...
      {dataManageOpen === true && (
        <div style={{
          ...
          <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>데이터 관리</span>
          </div>
          ...
          <div style={{ padding: '50px 40px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>데이터 백업</div>
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.6 }}>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>데이터 복원</div>
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.6 }}>백업 파일에서 데이터를 불러와요</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {dataManageOpen === 'records' && (
        <div style={{
          ...
          <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>측정 기록 관리</span>
          </div>
          ...
          <div style={{ padding: '50px 40px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>측정 기준 재설정</div>
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.6 }}>기준점을 처음부터 다시 측정해요</div>
              </div>
            </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 400, color: '#dc4444' }}>측정 기록 전체 삭제</div>
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.6 }}>모든 측정 기록과 사진을 삭제해요</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {restoreConfirm && (
        ...
        >
          ...
          >
            ...
            {!restoring ? (
              <>
                <div style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.6 }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button
                    ...
                    style={{
                      ...
                      background: 'transparent', color: 'var(--text-muted)', fontSize: 14, fontWeight: 400,
                      cursor: 'pointer', fontFamily: 'inherit',
                    }}
                  >취소</button>
                  <button
              ...
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)' }}>데이터를 복원하고 있어요...</div>
              </div>
            )}
          </div>
        </div>
      ...
      {baselineResetConfirm && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.7 }}>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.7 }}>
              ...
              <br />
              <span style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-dim)' }}>되돌릴 수 없어요. 등록한 화장품과 루틴 체크 기록·프로필은 유지됩니다.</span>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: 'calc(env(safe-area-inset-top, 0px) + 16px) 20px 0', display: 'flex', alignItems: 'center', position: 'relative' }}>
        ...
        <span style={{ position: 'absolute', left: 0, right: 0, textAlign: 'center', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>피부 목표</span>
      </div>
        ...
        <div style={{ padding: '50px 40px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
          {!latestRecord && (
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--accent-primary, var(--accent-primary))', lineHeight: 1.6 }}>
          ...
          {sortedMetrics.map((m, idx) => {
            ...
            return (
              ...
              >
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>{m.label}</span>
                    {idx < 3 && latestRecord && (
                      <span style={{
      ...
      {step === 2 && (
        <div style={{ padding: '30px 40px 0' }}>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  background: 'transparent',
                  color: 'var(--accent-primary, var(--accent-primary))', fontSize: 12, fontWeight: 400,
                  cursor: 'pointer', fontFamily: 'inherit',
                }}
              ...
              return (
                <div key={key} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>{meta.label}</span>
                    <span style={{ fontSize: 13, color: 'var(--accent-primary, var(--accent-primary))', fontWeight: 400 }}>
                      {current} → {target}
                    </span>
                  </div>
                  {/* Slider */}
          ...
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)', marginBottom: 10 }}>목표 기간</div>
            <div style={{ display: 'flex', gap: 8 }}>
              {[30, 60, 90].map((d) => (
                <button
                  key={d}
        ...
        <div style={{ padding: '30px 40px 0' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>목표 요약</span>
              <span style={{ fontSize: 12, color: 'var(--accent-primary, var(--accent-primary))', fontWeight: 400 }}>{duration}일</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {selectedMetrics.map((key) => {
                const meta = METRIC_META.find((m) => m.key === key);
                ...
                return (
                  <div key={key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-primary)' }}>{meta.label}</span>
          ...
          {existingGoal && existingGoal.status === 'active' && (
            <div style={{
              ...
              <div style={{ fontSize: 12, fontWeight: 400, color: 'var(--text-muted)', lineHeight: 1.5 }}>
                기존 목표가 새 목표로 대체됩니다.
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <button
              ...
              style={{
                ...
                color: 'var(--accent-primary, var(--accent-primary))', fontSize: 14, fontWeight: 400,
                cursor: 'pointer', fontFamily: 'inherit',
              }}
            >목표 시작하기</button>
          )}
...
function Section({ label, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-secondary)', marginBottom: 8 }}>{label}</div>
      {children}
    </div>
  );
}
      ...
      {options.map((opt) => {
        ...
        return (
          <div
            ...
            style={{
              padding: '7px 16px', borderRadius: 20, fontSize: 13, fontWeight: 400,
  ...
  return (
    <div style={{ textAlign: 'center', position: 'relative' }}>
      ...
      <div style={{
        ...
        {value}{unit && <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-muted)' }}>{unit}</span>}
      </div>
      <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400, letterSpacing: 0.3 }}>{label}</div>
    </div>
  );
}
...
function SettingsMenuItem({ icon, label, desc, right, onTap }) {
  return (
    <div onClick={onTap} style={{
      ...
      {right === 'badge-new' && (
        <span style={{
          padding: '3px 10px', borderRadius: 10, fontSize: 11, fontWeight: 400,
    ...
    <div style={{ borderTop: '1px solid var(--border-separator)' }}>
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          {!enabled && (
            <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 400, marginTop: 2 }}>꺼짐</div>
          )}
        </div>
        <div
...
function TimePicker({ value, onChange, onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ padding: '0 24px 12px', fontSize: 14, fontWeight: 400, color: 'var(--text-primary)' }}>
          알림 시간
        </div>
        <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          {times.map((t) => (
            <div
              ...
              style={{
                padding: '13px 24px',
                fontSize: 14, fontWeight: 400, color: t.h24 === currentH ? 'var(--accent-primary)' : 'var(--text-primary)',
