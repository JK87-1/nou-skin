...
const MISSION_DATABASE = {
  ...
  기본: {
    ...
    bonus: [
      ...
      { icon: '', category: '자외선', title: '선크림 꼼꼼히 바르기', description: '자외선 차단은 365일 필수예요.', xp: 15 },
    ],
  },
};
...
const UV_BONUS = {
  icon: '', category: '자외선', title: '선크림 꼼꼼히 바르기',
...
export function getTodayMissions(analysisResult) {
  ...
  // 자외선 차단 보너스 미션 보장
  const hasUV = bonus.some(b => b.icon === '');
  if (!hasUV) {
    bonus.push(UV_BONUS);
  }
