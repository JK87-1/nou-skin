/**
 * NOU Science Data v2.1 — Complete 10-Metric System
 */
...
export const SCIENCE = {
  ...
  skinAge: {
    ...
    methodology: `NOU의 피부 나이는 10개 피부 지표의 가중 합산으로 산출됩니다.
...
가중치 근거: Nkengne A (2008)의 연구에서 주름 > 처짐 > 색소 > 피부결 순으로 인지 연령에 영향을 준다고 보고했습니다.`,
    references: [
      { name: '10-Metric Weighted Age Model', description: '10개 피부 지표의 가중 패널티 합산으로 피부 나이를 산출. 최대 패널티 +32세, 우수 보정 -5세.', source: 'NOU Skin Analysis Engine v2.1' },
  ...
  moisture: {
    ...
    methodology: `NOU는 얼굴 각 영역의 밝기 표준편차(σ)를 직접 계산합니다.
...
Corneometer(각질층 전기 용량 수분 측정기)의 시각적 상관관계에 기반합니다.`,
    references: [
      { name: '밝기 표준편차(σ) 분석', description: '픽셀 밝기(L = 0.299R + 0.587G + 0.114B) 분산을 계산. σ↓ = 균일 보습.', source: 'NOU Computer Vision Engine' },
...
볼(홍조), 턱(호르몬성 여드름), 이마(피지성)를 구분 측정하고 GAGS 기준으로 심각도를 매핑합니다.`,
    references: [
      { name: '적색 우세 픽셀 카운팅', description: 'R>G×1.2 & R>B×1.25 & R>95 & (R-G)>15 조건으로 염증 감지.', source: 'NOU Computer Vision Engine' },
...
T존(이마+코) vs U존(볼+턱) 하이라이트 비율 비교로 유형을 판별합니다.`,
    references: [
      { name: '하이라이트 비율', description: '밝기>195 픽셀 = 유분 정반사.', source: 'NOU Computer Vision Engine' },
  ...
  wrinkles: {
    ...
    methodology: `NOU는 주름 3개 핵심 영역에서 방향성 에지 검출(Directional Edge Detection)을 수행합니다.
...
가중치: 이마(40%) + 눈가(35%) + 팔자(25%).`,
    references: [
      { name: '방향성 에지 검출', description: 'Laplacian 방향성 변형으로 주름 패턴만 선택적 검출.', source: 'NOU Computer Vision Engine' },
...
모공이 크면 5×5px 안에서 밝기 변화가 급격합니다. 코(50%) + 볼(50%) 동일 가중치.`,
    references: [
      { name: '마이크로 분산 분석', description: '5×5 슬라이딩 윈도우 내 밝기 분산. 분산↑ = 모공↑.', source: 'NOU Computer Vision Engine' },
...
현재 정면 1장 분석. 향후 측면 추가 시 정밀도 향상 예정.`,
    references: [
      { name: '턱선 에지 밀도', description: '턱~목 경계 에지 강도. 선명한 턱선 = 좋은 탄력.', source: 'NOU Computer Vision Engine' },
...
11×11 이웃의 평균 밝기 대비 15%+ 어두운 픽셀을 색소 침착으로 분류. 볼(60%) + 이마(40%) 합산.`,
    references: [
      { name: '국소 어두운 클러스터 검출', description: '11×11 이웃 대비 15%+ 어두운 픽셀 = 색소침착.', source: 'NOU Computer Vision Engine' },
...
이는 VISIA의 Texture 항목과 같은 원리입니다.`,
    references: [
      { name: '3×3 Laplacian 에너지', description: '중심 픽셀과 8개 이웃의 밝기 차이 합. 높은 에너지 = 거친 표면, 낮은 에너지 = 매끄러운 표면. 중간 주파수를 선택적으로 캡처합니다.', source: 'NOU Computer Vision Engine' },
      { name: '방향 구배 일관성 (Roughness)', description: '수평·수직 방향 밝기 변화율의 차이. 매끄러운 피부는 방향에 무관하게 균일하지만, 거친 피부는 방향에 따라 불규칙한 변화를 보입니다.', source: 'NOU Computer Vision Engine' },
...
좌·우 눈을 독립적으로 분석하고 평균합니다. 비대칭(좌우 차이)도 추적합니다.`,
    references: [
      { name: '밝기 차이 분석 (Dark Delta)', description: '눈 밑과 중간 볼의 밝기(Luminance) 차이를 정규화. 값이 높을수록 다크서클이 심합니다. 0: 없음, 0.1: 경미, 0.2+: 뚜렷.', source: 'NOU Computer Vision Engine' },
      { name: '블루 시프트 분석 (Blue Shift)', description: '눈 밑의 B/R 비율을 볼과 비교. 혈관형 다크서클은 얇은 피부 아래 정맥혈이 비치면서 푸른 색조를 나타냅니다. B/R 비율↑ = 혈관형 다크서클.', source: 'NOU Computer Vision Engine' },
      ...
      { name: 'Dermoscopy for Dark Circles', description: '피부경 관찰에서 혈관형은 보라-파랑 망상 패턴, 색소형은 갈색 균일 패턴을 보입니다. NOU의 블루 시프트 분석은 이 원리를 디지털로 근사합니다.', source: 'Freitag FM, Cestari TF (2007) J Cosmetic Dermatology' },
